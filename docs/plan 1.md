...
battle track   : dedupe_key = "m:{matchup_id}:s:{submission_id}:{identity}"
```
...
4. round.status = VOTING_OPEN, νέο voting window.
   → ψήφοι → votes.round_id + votes.matchup_id + votes.submission_id + choice (WIN/LOSS).
   → κάθε voter ψηφίζει W/L ανεξάρτητα για κάθε track του matchup.
   → ίδιο vote-validation pipeline (§6).
5. round κλείνει → matchup.winner = track με το υψηλότερο W ratio.
...
Frontend: bracket ως δέντρο· κάθε matchup = δύο tracks με ανεξάρτητα W/L controls.
